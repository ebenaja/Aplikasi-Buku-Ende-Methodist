...
class NrmMerahDetailPage extends StatefulWidget {
  final int index;
  final int totalPage;

  const NrmMerahDetailPage({
    super.key,
    required this.index,
  ...
  @override
  State<NrmMerahDetailPage> createState() => _NrmMerahDetailPageState();
}

class _NrmMerahDetailPageState extends State<NrmMerahDetailPage> {
  ...
  Future<void> _loadState() async {
    ...
    setState(() {
      _bookmarked = prefs.getBool('bookmark_nrm_merah_$_currentIndex') ?? false;
    });
  }

  Future<void> _saveLastRead() async {
    final prefs = await SharedPreferences.getInstance();
    prefs.setInt('last_read_nrm_merah', _currentIndex);
  }

  Future<void> _toggleBookmark() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() => _bookmarked = !_bookmarked);
    prefs.setBool('bookmark_nrm_merah_$_currentIndex', _bookmarked);
    ScaffoldMessenger.of(context).showSnackBar(
      ...
      builder: (_) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: primaryMerah),
              onPressed: () {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          children: [
            const Text("NRM Warna Merah", style: TextStyle(fontFamily: 'Serif', fontSize: 12, color: Colors.white70)),
